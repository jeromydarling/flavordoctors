...
test.describe.serial('Admin & Clinical Trials', () => {
  ...
  test('admin can edit a product price and revert it', async ({ page }) => {
    ...
    await priceInput.fill('9.49');
    await page.getByRole('button', { name: 'Save', exact: true }).click();
    await expect(page.getByRole('row', { name: /Ranch Rx/ }).getByText('$9.49')).toBeVisible();
    // Revert
    await page.getByRole('row', { name: /Ranch Rx/ }).getByRole('button', { name: 'Edit' }).click();
    await page.getByLabel('Price (USD)').fill('8.99');
    await page.getByRole('button', { name: 'Save', exact: true }).click();
    await expect(page.getByRole('row', { name: /Ranch Rx/ }).getByText('$8.99')).toBeVisible();
  });
  ...
  test('admin creates a Clinical Trial drop; it appears on /trials with waitlist', async ({ page }) => {
    ...
    await page.getByLabel('Name').fill('E2E Experimental Elixir');
    ...
    await page.getByRole('button', { name: 'Save', exact: true }).click();
    await expect(page.getByRole('cell', { name: /E2E Experimental Elixir/ })).toBeVisible();

    // Drop is on /trials, not on /menu
    await page.goto('/menu');
    await expect(page.getByRole('heading', { name: 'E2E Experimental Elixir' })).not.toBeVisible();
    await page.goto('/trials');
    await expect(page.getByRole('heading', { name: 'E2E Experimental Elixir' })).toBeVisible();
    ...
    await page.getByRole('row', { name: /E2E Experimental Elixir/ }).getByRole('button', { name: 'Deactivate' }).click();
    await expect(page.getByRole('row', { name: /E2E Experimental Elixir/ }).getByText('inactive')).toBeVisible();
  });

  test('admin orders view lists webhook-created orders and updates status', async ({ page }) => {
    const status = await sendWebhook(page.request, orderPaidEvent(null, 'guest-buyer@e2e.test', [{ p: 'p027', q: 1 }], 799));
    expect(status).toBe(200);
    await loginViaUi(page, ADMIN_EMAIL, ADMIN_PASSWORD);
    await page.goto('/admin/orders');
    const orderCard = page.locator('.rx-card').filter({ hasText: 'guest-buyer@e2e.test' });
    ...
    await page.reload();
    await expect(
      page.locator('.rx-card').filter({ hasText: 'guest-buyer@e2e.test' }).getByRole('combobox')
    ).toHaveValue('shipped');
  });
